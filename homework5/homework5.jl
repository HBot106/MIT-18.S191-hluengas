...
function make_tuple(coordinate)
...
function trajectory(w::Coordinate, n::Int)
	...
	return accumulate(+, rand(possible_moves, n))
end
...
# ╔═╡ eb7991e2-153d-11eb-345b-cff0052b51b6
function move(L::Number, c::Coordinate, m::Coordinate)
	collide_boundary(c+m, L)
end

# ╔═╡ 0665aa3e-0a69-11eb-2b5d-cd718e3c7432
function trajectory(c::Coordinate, n::Int, L::Number)
	
	accumulate(((a, b) -> collide_boundary(a+b, L)), rand(possible_moves, n), init=c)
end

# ╔═╡ 44107808-096c-11eb-013f-7b79a90aaac8
test_trajectory = trajectory(Coordinate(4,4), 100) # uncomment to test
...
# ╔═╡ 67b3c44a-153d-11eb-02ae-536af93b7293
let
	p = plot(ratio=1)
...
function visualize(agents::Vector, L::Number)
	p = scatter(ratio=1.0, xlims=(-L,L), ylims=(-L,L))
	
	
	
	scatter!(p, make_tuple.(position.(agents)), c=color.(agents), ratio=1.0, xlims=(-L,L), ylims=(-L,L))
...
# ╔═╡ 3001ca7e-1542-11eb-1a39-d1fa09c0b58d
function bernoulli(p::Number)
	return rand() < p
end

# ╔═╡ 7d47ebd8-1542-11eb-0359-53b04c594184
function is_susceptible(agent::Agent)
	agent.status == S
end

# ╔═╡ 9a5fe662-1542-11eb-1154-ff262d9834ea
function is_infected(agent::Agent)
	agent.status == I
end

# ╔═╡ 9c0a90f4-1542-11eb-049b-952c19880726
function is_recovered(agent::Agent)
	agent.status == R
end

# ╔═╡ 2ce8b126-1543-11eb-05af-07ef5b1ff114
...
function interact!(a::Agent, s::Agent, i::CollisionInfectionRecovery)

	if is_susceptible(a) && is_infected(s)
		
		if (a.position == s.position) && bernoulli(i.p_infection)
			set_status!(a,I)
		end

	elseif is_infected(a) && bernoulli(i.p_recovery)
		set_status!(a,R)
	end
end

# ╔═╡ 96c184ce-1543-11eb-27fe-edf5d0232fa9
let
	agent = Agent(Coordinate(0,0), S)
	source = Agent(Coordinate(0,0), I)
	infection = CollisionInfectionRecovery(0.5, 0.5)
...
function step!(agents::Vector{Agent}, L::Number, infection::AbstractInfection)
	...
	new_position = collide_boundary(source.position + rand(possible_moves), L)
...
# ╔═╡ df1087d6-1546-11eb-31aa-cb5a530b4088
...
# ╔═╡ 3eb85c8a-154d-11eb-3e13-039c2876cf66
sim_size = 1000

# ╔═╡ 69df7a4e-154b-11eb-0000-c37b0ea35c2c
function simulation()
	N = 50
	L = 30
	agents = initialize(N,L)
...
# ╔═╡ 0fb36948-154d-11eb-2f5a-250ab0fef1d8

sim = simulation()


# ╔═╡ 8441be76-1574-11eb-0a3c-e3351c20fafa
sim[3]

# ╔═╡ 1c227b86-154c-11eb-08c2-ef09affbaf3a
@bind time_t Slider(1:sim_size, default=1)

# ╔═╡ b43fa8ac-154b-11eb-1442-ad3eb6c809c0
let
	
	left = visualize(sim[1][time_t], 30)
	right = plot(xlim=(1,sim_size), ylim=(1,30), size=(600,300))
	
	plot!(right, 1:time_t, sim[2][1:time_t], color=color(S), label="S")
	plot!(right, 1:time_t, sim[3][1:time_t], color=color(I), label="I")
	plot!(right, 1:time_t, sim[4][1:time_t], color=color(R), label="R")
...
# ╔═╡ 801c8366-1572-11eb-2ba6-1df8547ba97e
let
    N = 50
    L = 30
