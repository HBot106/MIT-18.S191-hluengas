...
unused_letters = [c for c in alphabet if sample_freqs[index_of_letter(c)] == 0]
...
double_letters = [alphabet[i] 
	for i = 1:27 
	for j = 1:27
		if (i == j && sample_freq_matrix[i,j] > 0.0)]
...
# ╔═╡ f1931a62-0124-11eb-122f-73b0d17ce657
[(i,j) for (i,j) in enumerate(sample_freq_matrix[:,index_of_letter('w')])]

# ╔═╡ 0dd4a1ae-0126-11eb-1216-6f352387e19d
sort([(i,j) for (i,j) in enumerate(sample_freq_matrix[:,index_of_letter('w')])], by = x -> x[2], rev=true)
...
most_likely_to_follow_w = alphabet[
	sort(
		[(i,j) 
			for (i,j) in
		# 		'w' row
				enumerate(sample_freq_matrix[:,index_of_letter('w')])],
# 		sort in reverse order by the second tuple value (frequency)
		by = x -> x[2], rev=true)[1][1]]

# 	first value is most frequent, 
# 	first tuple value is index into alphabet
...
row_col_answer = md"""
the sum of each col is the freq of transitions to a letter

the sum of each row is the freq of transitions from a letter

the sum of both of these for any given i,j is the frequency of a letter
"""
...
function matrix_distance(A, B)
	sum(broadcast(abs, broadcast(-, A, B)))
end
...
function completions_cache(grams)
	...
	for gram in grams
		...
		if haskey(cache,key)
			cache[key] = append!(cache[key], [gram[length(gram)]])
		else
			cache[key] = [gram[length(gram)]]
		end
	end
