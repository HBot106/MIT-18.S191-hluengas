...
begin
	Pkg.add(["Plots", "PlutoUI", "StatsBase", "LsqFit", "Statistics"])
...
# ╔═╡ 0e229d54-072b-11eb-1552-db579dcd9e38
...
The patient is guaranteed to recover on the first day.
"""
...
function frequencies_plot_with_mean(data)
	...
	chart = bar(frequencies(data))
	
	vline!(chart, [mean(data)], label="Mean Recovery Time")
...
# ╔═╡ bb5597e8-072f-11eb-1137-8b95013b312d
md"p: $(@bind p_interactive Slider((0.01:0.01:1); default=0.5, show_value=true))"

# ╔═╡ bb63f3cc-042f-11eb-04ff-a128aec3c378
md"N: $(@bind N_interactive Slider((1000:1000:100_000); default=50_000, show_value=true))"

# ╔═╡ b4f5ddae-072f-11eb-31ce-316ec7ee3880
variable_experiment = do_experiment(p_interactive, N_interactive) 

# ╔═╡ 53e58784-0730-11eb-2688-976a366ff82c
frequencies_plot_with_mean(variable_experiment)
...
recovery_time_vs_p = [(p,mean(do_experiment(p, 10_000))) for p in 0.001:0.001:1]

# ╔═╡ c4195244-0733-11eb-2cd1-8b0ff2286895
plot(recovery_time_vs_p, xlabel="p - Recovery Probability", ylabel="τ - Mean Recovery Days")
...
# ╔═╡ 827ee382-0736-11eb-3aee-41cf016939dd
Integer(I)
...
# ╔═╡ 203d43ac-073c-11eb-0996-bbe9e28f58ed
typeof(generate_agents(3))
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	S_means = [mean(map(sim -> sim[1][i], simulations)) for i in 1:T]
	I_means = [mean(map(sim -> sim[2][i], simulations)) for i in 1:T]
	R_means = [mean(map(sim -> sim[3][i], simulations)) for i in 1:T]
	
	p = plot()
	plot!(p, 1:T, S_means, alpha=.5, label="Suceptable")
	plot!(p, 1:T, I_means, alpha=.5, label="Infected")
	plot!(p, 1:T, R_means, alpha=.5, label="Recovered")
...
# ╔═╡ f50528a4-074e-11eb-1198-9142172d9df9
md"Infection Rate: $(@bind interactive_infection Slider((.001:.001:1); default=.5, show_value=true))"

# ╔═╡ 0a73ad5a-074f-11eb-0268-e380991614c9
md"Recovery Rate: $(@bind interactive_recovery Slider((.001:.001:1); default=.5, show_value=true))"
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	I_means = [mean(map(sim -> sim[2][i], simulations)) for i in 1:T]
	R_means = [mean(map(sim -> sim[3][i], simulations)) for i in 1:T]
	S_std = [Statistics.std(map(sim -> sim[1][i], simulations)) for i in 1:T]
	I_std = [Statistics.std(map(sim -> sim[2][i], simulations)) for i in 1:T]
	R_std = [Statistics.std(map(sim -> sim[3][i], simulations)) for i in 1:T]
	
	p = plot()
	plot!(p, 1:T, S_means, alpha=.5, label="Suceptable", ribbon=S_std)
	plot!(p, 1:T, I_means, alpha=.5, label="Infected", ribbon=I_std)
	plot!(p, 1:T, R_means, alpha=.5, label="Recovered", ribbon=R_std)
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	# select agents
	rand_agent_1 = rand(agents)
	rand_agent_2 = rand(agents)
	
	# ensure they are different agents	
	while rand_agent_1 === rand_agent_2
		rand_agent_2 = rand(agents)
	end
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i in 1:T
		...
		for key in [S,I,R]
			append!(counts[key], count(a -> (a.status==key),agents))
			hcat
		end
	end
...
# ╔═╡ 9e335e24-0749-11eb-3bbd-8f35e864ba67
...
let
	...
	len = length(simulations[1][1])
	
	means = [mean(map(sim -> sim[2][i], simulations)) for i in 1:len]
	
	for sim in simulations
		plot!(p, 1:len, sim.I, alpha=.5, label=nothing)
	end
	plot!(p, 1:len, means, alpha=.5, label="mean", lw=5, color="black")
...
[map(sim -> sim[2][i], simulations) for i in 1:1000]

# ╔═╡ 1c6aa208-04d1-11eb-0b87-cf429e6ff6d0
sir_mean_plot(repeat_simulations(100, 1000, InfectionRecovery(interactive_infection, interactive_recovery), 20))
...
# ╔═╡ 5a1d53e4-0752-11eb-0b33-750008c5bcc1
simulations2 = repeat_simulations(100, 1000, Reinfection(0.02, 0.002), 20)

# ╔═╡ 1ac4b33a-0435-11eb-36f8-8f3f81ae7844
let
	p = plot()
	
	len = length(simulations2[1][1])
	
	means = [mean(map(sim -> sim[2][i], simulations2)) for i in 1:len]
